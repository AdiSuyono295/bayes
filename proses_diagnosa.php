...
<body>
	<div class="konten">
		<div style="text-align:center; background:linear-gradient(to top, #C33, #C90); color:#ffffff; font-family:Calibri; border-radius:50px 50px; height:25px; margin-bottom:8px;">
			...
			<a style="background-color:#99AB74;" href="index.php?top=home.php"><strong>BACK HOME</strong></a>&ensp;Hasil Diagnosa Penyakit Kacang Hijau
		</div>
		<?php
		include "koneksi.php";
		...
		<div style="float:left; position:inherit; width:320px;">
			<table width="330" height="158" border="1" style="float:left;">
				<tr>
					<td height="22">
						<h5>Hasil Analisa Diagnosa Penyakit Kacang Hijau</h5>
					</td>
				</tr>
				<tr>
					<td height="99">
						...
						while ($dataP = mysql_fetch_array($strP)) {
							...
							while ($dataGejala = mysql_fetch_array($strGejala)) {
								...
								echo "<br>";
								if (empty($dataGejalaBobot['bobot'])) {
									$dataGBobot = 0;
								} else {
									$dataGBobot = $dataGejalaBobot['bobot'];
								};
								...
								while ($dataH_nPenyakit = mysql_fetch_array($strH_nPenyakit)) {
									...
									echo "$NbobotGejalaR x " . $dataH_nPenyakit['nilai'];
									$nPH = $NbobotGejalaR * $dataH_nPenyakit['nilai'];
									echo "=$nPH)";
									$NilaiProbabilitasEvidencePH = $nPH;
									echo "[nilai evidence=" . $NilaiProbabilitasEvidencePH . "]";
								...
								echo "sum $dataSumTmpAnalisa[NilaiSumPH]";
								$nilaiSum = $dataSumTmpAnalisa['NilaiSumPH'];
								...
								//jumlahkan h1*g1
								$bobotG = $dataGejalaBobot['bobot'];
								$bobotP = $dataProbabilitasPHE['nilai'];
								$NilaiPHAtas = $bobotG * $bobotP;
		...
		<div style="float:left; min-height:500px; margin-left:20px;">
			<table width="430" border="0" bgcolor="#0099FF" cellspacing="1" cellpadding="4" bordercolor="#0099FF">
				<tr bgcolor="#ffffff">
					<td width="408" height="32" style="color:#C60;">
						...
						echo "<p style='color:#06F; font-size:13pt; font-weight:bold; font-family:Times New Roman;'>Data User :</p> ";
		  ...
		  <tr>
			...
			<td>$data_pasien[email]</td>
						...
						echo "<p style='color:#06F; font-size:13pt; font-weight:bold; font-family:Times New Roman;'>Berdasarkan Gejala Yang Di Input User:</p> ";
						$query_gejala_input = mysql_query("SELECT tbgejala.gejala AS namagejala,tmp_gejala.kd_gejala FROM tbgejala,tmp_gejala WHERE tmp_gejala.kd_gejala=tbgejala.kd_gejala");
						$nogejala = 0;
						while ($row_gejala_input = mysql_fetch_array($query_gejala_input)) {
							$nogejala++;
